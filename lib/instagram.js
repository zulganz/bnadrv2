...
import fakeUa from '../controller/fakeUa';
		...
		return new Promise(async (resolve, reject) => {
			await axios.request({
				...
				const result = {
					...
					followers: functions.h2k(metaData.edge_followed_by.count) + ' Followers',
					posts_count: functions.h2k(metaData.edge_owner_to_timeline_media.edges.map(x => x.node).length) + ' Posts',
