...
class Instagram {
	constructor() {
		this.headers = {
			...
			'Cookie': `mid=ZPachAALAAH8A6a6XeBXeqTav6S5; datr=zJz2ZGRxuCYkBPm1M2e8-Nun; csrftoken=LOk3b4AsMiIzZKMHnrnBf7w5khLvAMbX; ds_user_id=11894486394; dpr=1.25; shbid="11711\05411894486394\0541732780037:01f761e836b580017d38b0b5f6dc40fdc2abc352c97b8e8c8fb4dce5475b6888d6223f41"; shbts="1701244037\05411894486394\0541732780037:01f7cf40cc8fea6dd21020fe825282e6460e4e607b1f648463ed6e4b8af6cca6f38a5fca"; sessionid=11894486394%3ARnbat71HPlAPTg%3A1%3AAYeAIoo66TMrFrEqvWjiQU-Z3dGPkP590xvOrDAwuA; rur="EAG\05411894486394\0541732805239:01f7b3e77f90e564927350a501ce75ed655034d39ecbfbd1636970e47b1b8af0b21563b7"`,
					...
					if (metadata.carousel_media_count) {
						...
						for (let i = 0; i < media_count; i++) {
							result_url.push({
								...
								url: metadata.carousel_media[i].image_versions2.candidates.filter(fill => fill.width === 1080)[0].url
							});
						}
						resolve({
	...
	stories(url) {
		return new Promise(async (resolve, reject) => {
			...
			let { data, statusCode, statusMessage } = await axios.get('https://storiesdown.com/users/' + getUsername, {
				headers: {
			...
			let result = $("script#__NEXT_DATA__").get().map(m => {
				let results;
	...
	story(username) {
		return new Promise(async (resolve, reject) => {
			axios.request({
				...
				axios.post('https://www.instagramsave.com/system/action.php', config.data, {
